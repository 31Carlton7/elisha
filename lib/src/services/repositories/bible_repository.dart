...
class BibleRepository {
  ...
  Future<void> goToNextPreviousChapter(
      BuildContext context, bool isBackward) async {
    bool isFirstChapterInBook() {
      return context.read(localRepositoryProvider).chapter == 1;
    }

    bool isGenesisOne() {
      if (context.read(localRepositoryProvider).book == 1 &&
          context.read(localRepositoryProvider).chapter == 1) {
        return true;
      }
      return false;
    }
    ...
    bool isRevelationTwentyTwo() {
      if (context.read(localRepositoryProvider).book == 66 &&
          context.read(localRepositoryProvider).chapter == 22) {
        return true;
      }
      return false;
    }
  ...
  Future<void> changeChapter(
      BuildContext context, int book, int chapter) async {
